...
const CrewReport: FC<IProps> = ({ data, previewReport, setPreviewReport }) => {
  ...
  const rowData = data.map((item: any, i: number) => ({
    No: i + 1,
    Nama: `${item.givenName} ${item.surName}`,
    Alamat: item.homeAddress,
    'Tgl Lahir': new Date(item.dob).toLocaleDateString(),
    'No Telepon': item.phoneNumber,
    'Main Rank': item.mainRank,
    Status: item.status,
  }))
